...
- [DistributedKvStore](#distributedkvstore)
  ...
  - [Poking at it with Postman](#poking-at-it-with-postman)
...
If `docker compose logs cluster-init` shows the script failing immediately, check that `docker/init-cluster.sh` still has LF line endings — `/bin/sh` in the container chokes on CRLF. `.gitattributes` pins `*.sh` to LF so a Windows checkout doesn't convert it.

## Poking at it with Postman

`postman/` holds a collection and a matching environment covering both surfaces — TestApi `/key/*` and `/cluster/*`, and the per-node `api/data/*`, `api/cluster/*` and `internal/*` routes.

Import `DistributedKvStore.postman_collection.json` and `DistributedKvStore.postman_environment.json`, select the **DistributedKvStore - Docker Compose** environment, and start with `TestApi > Key-Value > Create Key`.

Two kinds of URL matter here: `http://localhost:500N` is how *you* reach a node, while `http://nodeN:8080` is how the containers reach each other. Anything the server dials on your behalf — `baseUrl` in Add Node, `?baseUrl=` on Get Node Health and Get Node Cluster State — needs the in-network form. The environment ships both as `nodeNBaseUrl` and `nodeNInternalUrl`.
